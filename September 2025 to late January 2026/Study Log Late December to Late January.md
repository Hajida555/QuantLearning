...
## 12-30
1. 矩阵的四种乘法与逆矩阵的判断和运算
  - 点乘法 拆开成列 拆开成小矩阵 行乘列 列乘行
  - 矩阵可以找到另一个的x相乘为0 即为不可逆
  - 也可以通过行列式计算 为0 也不可逆
2. 在github上配置了一个自动传输热点的工具
3. 逆矩阵、列空间与零空间
  - 过3b1b视频不懂 
  - 考虑多看 或者看mit到相关章节辅助学习
4. 读写文本格式的数据
  - github下载书籍数据
  - read_csv skiprows去行
  - 对于不同的缺失值的处理
  - keep_default_na=False 让缺失值失效 可补充不想失效的缺失值
5. 反思
  - 看书走神过多 应该在上课的时候看书 没课多动
  - 花费在逛github时间过多 严重占有学习时间
  - 急于证明 在学习时间中 突然想为其他人作一些脚本工具 应该在休息时间仔细考虑 再动 
  - 还花费部分时间在作数学题上 无考试收益低 应该与量化实操结合
  - 最近停止了睡前看书 换听博客 怀疑是否有影响注意力 每天更换回看书