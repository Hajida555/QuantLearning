...
1. 宋浩概率论 
 ...
 - 事件独立 P(AB)=P(A)P(B)
2. 《利用python数据分析》
 - .figure绘画布 add_subplot(子像行数，子图列数，第几个)在此基础上创建子图 
 - .hist/scatter/plot 直方图（统计一组数值型数据在不同取值区间的出现次数）/散点图/折线图
 - 通过subplots赋值给图表名A，子图列表B 
 - cumsum() 累加 linestyle=线条形状"dashed"虚线 bins=以多少分组 alpha=颜色深度0.1-1
 - marker="o" 将折线图的数据点标记设置为圆形
## 1-18
1. 宋浩概率论 
 - 对立事件还独立 
 - 两种结果伯努利 发生n次 一共n次 求其中发生m次 从总共发生n次中选择出m次再 * p^k * (1-p)^n-m
2. MIT + 《线性代数导论》
 - Ax平行x 则x为特征向量 即Ax=系数*x
 - 如果A是奇异矩阵与向量x相乘的结果是零 说明特征值是零
 - 投影矩阵作用在平面上的任意x 还是x 即Px=x 作用垂直于平面的向量的结果为0 即 Px=0 特征值是1/0
 - Ax=系数*x 转化为(A-系数I)x=0 可以得知(A-系数I)是奇异矩阵 因为Ax=0 不为零则A为奇异矩阵 
 - 由奇异矩阵的特征的行列式的值为零 可得det(A-系数I)=0 即特征方程 当对A平方时 特征值也平法 特征向量保持不变
 - n*n大概有n个特征值 特征值的和=对角线的和
3. 《利用python数据分析》
 - label=命名图例 .legend() 显示图例 drawstyle改变折线的走法 
 - set_x/yticks 改变坐标轴的大小分类 set_x/yticklabels 改变坐标轴上的名称 set_x/ylabel 改变坐标轴的名称 set_title 改变表的名字
 - annotabl对图表标记(标记的内容, 箭头的终点=（x，y）,标记数据的位置=（x，y），arrowprops箭头样式=dict字典（facecolor=颜色, headwidth=头部的粗细, width=箭身的大小, headlength=头部的像素）)
